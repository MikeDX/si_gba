...
//	mem=0x3000000;
	...
	for(xloop=0x0000;xloop<0x8000;xloop++) mem[xloop]=0;
	if ( game==0)
	{
	...
	{
		dwResult = mz80exec(INT_INTERVAL);
		Interrupt();
...
void writemem(UINT32 addr, UINT8 val, struct MemoryWriteByte *psWrite)
{
	...
	PlotPixel(xpos,ypos++, ((val>>6)&1) ? COLOR : invadersBitmap[mpos+=240]);

	PlotPixel(xpos,ypos++, ((val>>4)&1) ? COLOR : invadersBitmap[mpos+=240]);
	PlotPixel(xpos,ypos++, ((val>>3)&1) ? COLOR : invadersBitmap[mpos+=240]);

	PlotPixel(xpos,ypos++, ((val>>1)&1) ? COLOR : invadersBitmap[mpos+=240]);
	PlotPixel(xpos,ypos++, ((val>>0)&1) ? COLOR : invadersBitmap[mpos]);
}
