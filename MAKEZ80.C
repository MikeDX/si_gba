...
static char u8CpuBaseName[150];
static char u8mz80Index[50];
static char u8mz80IndexHalfHigh[50];
static char u8mz80IndexHalfLow[50];
static char u8MajorOp[50];
static char u8ProcName[150];
static UINT32 u32GlobalLabel = 0;
...
static UINT8 u8Plain = FALSE;
static UINT8 u8NoTiming = FALSE;
static UINT8 u8UseStack = 0;
static UINT8 u8CurrentMode = TIMING_REGULAR;	// Current timing mode
static UINT8 u816BitIO = FALSE;
static UINT8 u8ThroughCallHandler = FALSE;
static UINT8 u8OS2 = FALSE;
static UINT8 u88085Mode = FALSE;
static UINT8 u8What = MZ80_UNKNOWN;
static UINT8 u8StaticFlagTable = FALSE;
static void ProcBegin(void);

static const UINT8 *pu8LocalReg[8] =
{	
	(UINT8 *) "ch",
	(UINT8 *) "cl",
	(UINT8 *) "dh",
...
static const UINT8 *pu8LocalRegC[8] =
{
	(UINT8 *) "cpu.z80B",
	(UINT8 *) "cpu.z80C",
	(UINT8 *) "cpu.z80D",
...
static const UINT8 *pu8RegPairC[] =
{
	(UINT8 *) "cpu.z80BC",
	(UINT8 *) "cpu.z80DE",
	(UINT8 *) "cpu.z80HL",
...
static const UINT8 *pbFlagsC[8] =
{
	(UINT8 *) "(!(cpu.z80F & Z80_FLAG_ZERO))",
	(UINT8 *) "(cpu.z80F & Z80_FLAG_ZERO)",
	(UINT8 *) "(!(cpu.z80F & Z80_FLAG_CARRY))",
	(UINT8 *) "(cpu.z80F & Z80_FLAG_CARRY)",
	(UINT8 *) "(!(cpu.z80F & Z80_FLAG_OVERFLOW_PARITY))",
	(UINT8 *) "(cpu.z80F & Z80_FLAG_OVERFLOW_PARITY)",
	(UINT8 *) "(!(cpu.z80F & Z80_FLAG_SIGN))",
	(UINT8 *) "(cpu.z80F & Z80_FLAG_SIGN)"
};

static const UINT8 *pbMathReg[8] =
{
	(UINT8 *) "ch",
	(UINT8 *) "cl",
	(UINT8 *) "byte [_z80de + 1]",
...
static const UINT8 *pbMathRegC[8] =
{
	(UINT8 *) "cpu.z80B",
	(UINT8 *) "cpu.z80C",
	(UINT8 *) "cpu.z80D",
...
static const UINT8 *pu8RegPairs[4] = 
{
	(UINT8 *) "cx",	// BC
	(UINT8 *) "word [_z80de]", // DE
	(UINT8 *) "bx",	// HL
	(UINT8 *) "word [_z80sp]"  // SP
};

static const UINT8 *pu8RegPairsC[4] = 
{
	(UINT8 *) "cpu.z80BC",	// BC
	(UINT8 *) "cpu.z80DE", // DE
	(UINT8 *) "cpu.z80HL",	// HL
...
static const UINT8 *pbPopRegPairs[4] = 
{
	(UINT8 *) "cx",	// BC
	(UINT8 *) "word [_z80de]", // DE
	(UINT8 *) "bx",	// HL
...
static const UINT8 *pbPopRegPairC[4] = 
{
	(UINT8 *) "cpu.z80BC",
	(UINT8 *) "cpu.z80DE",
	(UINT8 *) "cpu.z80HL",
...
static const UINT8 *pbIndexedRegPairs[4] = 
{
	(UINT8 *) "cx",	// BC
	(UINT8 *) "word [_z80de]", // DE
	(UINT8 *) "di",	// IX/IY
...
static const UINT8 u8TimingRegular[0x100] =
{
...
static const UINT8 u8TimingCB[0x100] =
{
...
static const UINT8 u8TimingXXCB[0x100] =
{
...
static const UINT8 u8TimingDDFD[0x100] =
{
...
static const UINT8 u8TimingED[0x100] = 
{
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 
...
static void EDHandler(UINT8 u8Opcode);
static void DDHandler(UINT8 u8Opcode);
static void FDHandler(UINT8 u8Opcode);
static void CBHandler(UINT8 u8Opcode);

static void PushPopOperations(UINT8 u8Opcode);
static void AddRegpairOperations(UINT8 u8Opcode);
static void CallHandler(UINT8 u8Opcode);
static void MiscHandler(UINT8 u8Opcode);
static void IMHandler(UINT8 u8Opcode);
static void IRHandler(UINT8 u8Opcode);
static void LdRegPairImmediate(UINT8 u8Opcode);
static void LoadImmediate(UINT8 u8Opcode);
static void LdRegpairPtrByte(UINT8 u8Opcode);
static void MathOperation(UINT8 u8Opcode);
static void RegIntoMemory(UINT8 u8Opcode);
static void JpHandler(UINT8 u8Opcode);
static void LdRegImmediate(UINT8 u8Opcode);
static void IncRegister(UINT8 u8Opcode);
static void DecRegister(UINT8 u8Opcode);
static void IncDecRegpair(UINT8 u8Opcode);
static void LdRegReg(UINT8 u8Opcode);
static void MathOperationDirect(UINT8 u8Opcode);
static void JrHandler(UINT8 u8Opcode);
static void RetHandler(UINT8 u8Opcode);
static void RestartHandler(UINT8 u8Opcode);
static void ToRegFromHl(UINT8 u8Opcode);
static void RraRlaHandler(UINT8 u8Opcode);
static void LdByteRegpair(UINT8 u8Opcode);
static void IncDecHLPtr(UINT8 u8Opcode);
static void InOutHandler(UINT8 u8Opcode);
static void RLCRRCRLRRSLASRASRLHandler(UINT8 u8Opcode);
static void BITHandler(UINT8 u8Opcode);
static void RESSETHandler(UINT8 u8Opcode);
static void PushPopOperationsIndexed(UINT8 u8Opcode);
static void LDILDRLDIRLDDRHandler(UINT8 u8Opcode);
static void LdRegpair(UINT8 u8Opcode);
static void ExtendedRegIntoMemory(UINT8 u8Opcode);
static void NegHandler(UINT8 u8Opcode);
static void ExtendedInHandler(UINT8 u8Opcode);
static void ExtendedOutHandler(UINT8 u8Opcode);
static void RetIRetNHandler(UINT8 u8Opcode);
static void AdcSbcRegpair(UINT8 u8Opcode);
static void CPICPDCPIRCPDRHandler(UINT8 u8Opcode);
static void RRDRLDHandler(UINT8 u8Opcode);
static void UndocRegToIndex(UINT8 u8Opcode);
static void UndocIndexToReg(UINT8 u8Opcode);
static void MathOperationIndexed(UINT8 u8Opcode);
static void IncDecIndexed(UINT8 u8Opcode);
static void DDFDCBHandler(UINT8 u8Opcode);
static void JPIXIYHandler(UINT8 u8Opcode);
static void AddIndexHandler(UINT8 u8Opcode);
static void SPToIndex(UINT8 u8Opcode);
static void LdByteToIndex(UINT8 u8Opcode);
static void LdRegIndexOffset(UINT8 u8Opcode);
static void IncDecIndexReg(UINT8 u8Opcode);
static void ExIndexed(UINT8 u8Opcode);
static void UndocIncDecIndexReg(UINT8 u8Opcode);
static void UndocLoadHalfIndexReg(UINT8 u8Opcode);
static void UndocMathIndex(UINT8 u8Opcode);
static void ddcbBitWise(UINT8 u8Opcode);
static void LdIndexPtrReg(UINT8 u8Opcode);
static void StoreIndexReg(UINT8 u8Opcode);
static void LoadIndexReg(UINT8 u8Opcode);
static void OTIROTDROUTIOUTDHandler(UINT8 u8Opcode);
static void INIRINDRINIINDHandler(UINT8 u8Opcode);
...
static const struct sOp StandardOps[] =
{
	{0xd3,  InOutHandler},		// V
	{0xdb,  InOutHandler},		// V
...
static const struct sOp DDFDOps[] =
{
	{0x35,  IncDecIndexed},
	{0x34,  IncDecIndexed},
 	{0xcb,  DDFDCBHandler},
...
static const struct sOp DDFDCBOps[] =
{
	{0x06,  ddcbBitWise},
	{0x0e,  ddcbBitWise},
	{0x16,  ddcbBitWise},
...
static void InvalidInstructionC(UINT32 u32Count)
{
	fprintf(fp, "				InvalidInstruction(%ld);\n", u32Count);
}

static UINT32 Timing(UINT8 bWho, UINT8 u8Opcode)
{
	UINT32 u32Timing = 0;

	assert(u8Opcode < 0x100);

	if (TIMING_REGULAR == bWho)	// Regular?
		u32Timing = u8TimingRegular[u8Opcode];
	else
	if (TIMING_CB == bWho)
		u32Timing = u8TimingCB[u8Opcode];
	else
	if (TIMING_DDFD == bWho)
		u32Timing = u8TimingDDFD[u8Opcode];
	else
	if (TIMING_ED == bWho)
		u32Timing = u8TimingED[u8Opcode];
	else
	if (TIMING_XXCB == bWho)
 		u32Timing = u8TimingXXCB[u8Opcode];
	else
...
static void IndexedOffset(UINT8 *Localu8mz80Index)
{
...
static void CBHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		fprintf(fp, ";\n");
		fprintf(fp, "; Handler for all CBxx instructions\n");
		fprintf(fp, ";\n");
		sprintf((char *) string, "RegInst%.2x", u8Opcode);
		ProcBegin();

		if (u88085Mode)
		{
			fprintf(fp, "		jmp	invalidInsByte\n\n");
			return;
		}
...
static void StandardHeader(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
...
static void SetSubFlagsSZHVC(UINT8 *pu8Left, UINT8 *pu8Right)
{
...
static void SetSbcFlagsSZHVC(UINT8 *pu8Left, UINT8 *pu8Right)
{
	fprintf(fp, "				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | \n");
	fprintf(fp, "							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |\n");
...
static void SetAddFlagsSZHVC(UINT8 *pu8Left, UINT8 *pu8Right)
{
	fprintf(fp, "				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | \n");
	fprintf(fp, "							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |\n");
...
static void SetAdcFlagsSZHVC(UINT8 *pu8Left, UINT8 *pu8Right)
{
	fprintf(fp, "				cpu.z80F = (cpu.z80F & ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | \n");
	fprintf(fp, "							   Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN)) |\n");
...
static void FetchNextInstruction(UINT8 u8Opcode)
{
	if (0xffffffff != u8Opcode)
	{
		fprintf(fp, "		sub	edi, byte %ld\n", Timing(u8CurrentMode, u8Opcode));
		
		if (u8CurrentMode == TIMING_REGULAR)
...
static void WriteValueToMemory(UINT8 *pu8Address, UINT8 *pu8Value)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (strcmp((const char *) pu8Value, "al") != 0)
			fprintf(fp, "		mov	al, %s	; And our data to write\n", pu8Value);
		if (strcmp((const char *) pu8Value, (const char *) "[esi]") == 0)	// Immediate value?
			fprintf(fp, "		inc	esi	; Increment our program counter\n");

		// Now get the address in DX - regardless of what it is

		if (strcmp((const char *) pu8Address, "[_z80de]") == 0 ||
			 strcmp((const char *) pu8Address, "[_orgval]") == 0 ||
			 strcmp((const char *) pu8Address, "[_z80ix]") == 0 ||
			 strcmp((const char *) pu8Address, "[_z80iy]") == 0)
			fprintf(fp, "		mov	dx, %s\n", pu8Address);
		...
		if ((strcmp((const char *) pu8Address, "dx") != 0) && (strcmp((const char *) pu8Address, "[_z80de]") != 0) &&
			 (strcmp((const char *) pu8Address, "[_z80ix]") != 0) &&
			 (strcmp((const char *) pu8Address, "[_orgval]") != 0) &&
			 (strcmp((const char *) pu8Address, "[_z80iy]") != 0))
		...
		if (strcmp((const char *) pu8Value, "[esi]") == 0)
			fprintf(fp, "		mov	[ebp + e%s], al	; Store our direct value\n", pu8Address);
		else
		{
			if (pu8Value[0] == 'b' && pu8Value[1] == 'y' && pu8Value[2] == 't')
			{
				fprintf(fp, "		mov	edi, edx\n");
				assert(strcmp((const char *) pu8Value, "dl") != 0);
	
				fprintf(fp, "		mov	dl, %s\n", pu8Value);
	
				if (strcmp((const char *) pu8Address, "dx") == 0)
...
static void WriteWordToMemory(UINT8 *pu8Address, UINT8 *pu8Target)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		if (strcmp((const char *) pu8Target, "ax") != 0)
		{
			fprintf(fp, "		mov	ax, %s\n", pu8Target);
		}
		else
		...
		if (strlen((const char *) pu8Target) != 2)
		{
			fprintf(fp, "		mov	di, %s\n", pu8Target);
...
static void WriteValueToIo(UINT8 *pu8IoAddress, UINT8 *pu8Value)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		if (strcmp((const char *) pu8Value, "al") != 0)
			fprintf(fp, "		mov	al, %s	; And our data to write\n", pu8Value);
		if (strcmp((const char *) pu8Value, "[esi]") == 0)	// Immediate value?
			fprintf(fp, "		inc	esi	; Increment our program counter\n");
		...
		if (strcmp((const char *) pu8IoAddress, "dx") != 0)
			fprintf(fp, "		mov	dx, %s	; Get our address to target\n", pu8IoAddress);
...
static void ReadValueFromMemory(UINT8 *pu8Address, UINT8 *pu8Target)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		if (strcmp((const char *) pu8Target, "al") == 0)
			fprintf(fp, "		mov	[_z80af], al	; Save our new accumulator\n");
		else
		if (strcmp((const char *) pu8Target, "ah") == 0)
		...
		if (pu8Target[0] == 'b' && pu8Target[1] == 'y' && pu8Target[2] == 't')
		{
...
static void ReadWordFromMemory(UINT8 *pu8Address, UINT8 *pu8Target)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		if (strcmp((const char *) pu8Target, "ax") != 0)
		{
			...
			if (strcmp((const char *) pu8Target, "dx") != 0)
			{
...
static void ReadValueFromIo(UINT8 *pu8IoAddress, UINT8 *pu8Target)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		fprintf(fp, "		mov	[cyclesRemaining], edi\n");
		...
		if (strcmp((const char *) pu8Target, "dl") == 0)
			fprintf(fp, "		mov	[_z80de], al	; Put it in E\n");
		else
		if (strcmp((const char *) pu8Target, "dh") == 0)
...
static void MiscHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x2a)
		{
			...
			ReadWordFromMemory((UINT8 *) "dx", (UINT8 *) "bx");
			fprintf(fp, "		xor	edx, edx\n");
		}
		...
		FetchNextInstruction(u8Opcode);
	}
	else
	if (MZ80_C == u8What)
	{
		if (u8Opcode == 0x76)		// HALT!
		{
		...
		if (u8Opcode == 0xf9)		// LD SP, HL
		{
			fprintf(fp, "				cpu.z80sp = cpu.z80HL;\n");
		}
		else
		if (u8Opcode == 0x27)		// DAA
		{
		...
		if (u8Opcode == 0x2a)
		{
			...
			ReadWordFromMemory((UINT8 *) "dwAddr", (UINT8 *) "cpu.z80HL");
		}
		else
		if (u8Opcode == 0xe3)		// EX (SP), HL
		{
			ReadWordFromMemory((UINT8 *) "cpu.z80sp", (UINT8 *) "dwAddr");
			WriteWordToMemory((UINT8 *) "cpu.z80sp", (UINT8 *) "cpu.z80HL");
			fprintf(fp, "				cpu.z80HL = dwAddr;\n");
		}
		else
		if (u8Opcode == 0xe9)		// JP (HL)
		{
			fprintf(fp, "				pbPC = cpu.z80Base + cpu.z80HL;\n");
		}
		else
		if (0x08 == u8Opcode)		// EX AF, AF'
		{
		...
		if (0x3a == u8Opcode)	// LD A, (xxxxh)
		{
			fprintf(fp, "				dwTemp = *pbPC++;\n");
			fprintf(fp, "				dwTemp |= (((UINT32) *pbPC++) << 8);\n");
			ReadValueFromMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80A");
		}
		else
		if (0xf3 == u8Opcode)	// DI
		{
...
static void LdRegpairPtrByte(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x36)	// Immediate into (HL)
			WriteValueToMemory((UINT8 *) "bx", (UINT8 *) "[esi]");

		if (u8Opcode == 0x12)
			WriteValueToMemory((UINT8 *) "[_z80de]", (UINT8 *) "al");	// (DE), A

		if (u8Opcode == 0x2)		// (BC), A
			WriteValueToMemory((UINT8 *) "cx", (UINT8 *) "al");

		if (u8Opcode >= 0x70 && u8Opcode < 0x78)
			WriteValueToMemory((UINT8 *) "bx", (UINT8 *) pbMathReg[u8Opcode & 0x07]);
	...
	{
		if (u8Opcode == 0x36)
			WriteValueToMemory((UINT8 *) "cpu.z80HL", (UINT8 *) "*pbPC++");

		if (u8Opcode == 0x12)
			WriteValueToMemory((UINT8 *) "cpu.z80DE", (UINT8 *) "cpu.z80A");

		if (u8Opcode == 0x02)
			WriteValueToMemory((UINT8 *) "cpu.z80BC", (UINT8 *) "cpu.z80A");

		if (u8Opcode >= 0x70 && u8Opcode < 0x78)
			WriteValueToMemory((UINT8 *) "cpu.z80HL", (UINT8 *) pbMathRegC[u8Opcode & 0x07]);
	}
	else
	{
		assert(0);
...
static void MathOperation(UINT8 u8OrgOpcode)
{
	UINT8 bRegister;
	UINT8 u8Opcode;
	UINT8 tempstr[150];
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		u8Opcode = u8OrgOpcode;
		bRegister = (UINT8) (u8Opcode & (UINT8) 0x07);
		u8Opcode &= 0xf8;

		if (u8Opcode == 0x80)
			strcpy((char *) tempstr, "add");
		if (u8Opcode == 0x88)
			strcpy((char *) tempstr, "adc");
		if (u8Opcode == 0x90)
			strcpy((char *) tempstr, "sub");
		if (u8Opcode == 0x98)
			strcpy((char *) tempstr, "sbb");
		if (u8Opcode == 0xa0)
			strcpy((char *) tempstr, "and");
		if (u8Opcode == 0xa8)
			strcpy((char *) tempstr, "xor");
		if (u8Opcode == 0xb0)
			strcpy((char *) tempstr, "or");
		if (u8Opcode == 0xb8)
			strcpy((char *) tempstr, "cmp");
		...
		if (bRegister == 0x6)
		{
			...
			ReadValueFromMemory((UINT8 *) "bx", (UINT8 *) "dl");
		}
		...
		if (u8Opcode != 0xa8 && u8Opcode != 0xa0 && u8Opcode != 0xb0)
			SetOverflow();
		...
		if (u8Opcode == 0x80 || u8Opcode == 0x88)
			fprintf(fp, "		and	ah, 0fdh ; No N!\n");
	
		if (u8Opcode == 0x90 || u8Opcode == 0x98)
			fprintf(fp, "		or	ah, 02h	; N Gets set!\n");

		if (bRegister == 0x6)
			fprintf(fp, "		xor	edx, edx	; Zero this...\n");
	
		FetchNextInstruction(u8OrgOpcode);
	}
	else
	if (MZ80_C == u8What)
	{
		u8Opcode = u8OrgOpcode;
		bRegister = (UINT8) (u8Opcode & 0x07);
		u8Opcode &= 0xf8;

		if (6 == bRegister)		// Deal with (HL)
		{
			ReadValueFromMemory((UINT8 *) "cpu.z80HL", (UINT8 *) "bTemp");
		}
		...
		if (u8Opcode == 0x88)		// ADC
		{
			fprintf(fp, "				bTemp2 = cpu.z80A + %s + (cpu.z80F & Z80_FLAG_CARRY);\n", pbMathRegC[bRegister]);
		}
		else
		if (u8Opcode == 0x90)		// SUB
		{
			fprintf(fp, "				bTemp2 = cpu.z80A - %s;\n", pbMathRegC[bRegister]);
		}										  
		else
		if (u8Opcode == 0x80)		// ADD
		{
			fprintf(fp, "				bTemp2 = cpu.z80A + %s;\n", pbMathRegC[bRegister]);
		}
		else
		if (u8Opcode == 0x98)		// SBC
		{
			fprintf(fp, "				bTemp2 = cpu.z80A - %s - (cpu.z80F & Z80_FLAG_CARRY);\n", pbMathRegC[bRegister]);
		}
		else
		...
		if (0xb0 == u8Opcode || 0xa8 == u8Opcode)
		{
		...
		if (0xb8 == u8Opcode || 0x90 == u8Opcode)
		{
			SetSubFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) pbMathRegC[bRegister]);
		...
		if (0x80 == u8Opcode)		// Add fixup
		{
			SetAddFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) pbMathRegC[bRegister]);
			fprintf(fp, "				cpu.z80A = bTemp2;\n");
		}

		if (0x88 == u8Opcode)		// Adc fixup
		{
			SetAdcFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) pbMathRegC[bRegister]);
			fprintf(fp, "				cpu.z80A = bTemp2;\n");
		}

		if (0x98 == u8Opcode)		// Sbc fixup
		{
			SetSbcFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) pbMathRegC[bRegister]);
			fprintf(fp, "				cpu.z80A = bTemp2;\n");
		}
	}
	else
...
static void RegIntoMemory(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (0x32 == u8Opcode)		// LD (xxxx), A
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "al");
		if (0x22 == u8Opcode)		// LD (xxxx), HL
		{
			WriteWordToMemory((UINT8 *) "dx", (UINT8 *) "bx");
		}
	...
	if (MZ80_C == u8What)
	{
		...
		if (0x32 == u8Opcode)
			WriteValueToMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80A");
		if (0x22 == u8Opcode)
			WriteWordToMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80HL");
...
static void JpHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (0xc3 == u8Opcode)	// If it's a straight jump...
		{
...
static void IncRegister(UINT8 u8Opcode)
{
	...
	u8Opcode1 = (UINT8) ((u8Opcode >> 3) & 0x07);
...
static void IncDecRegpair(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if ((u8Opcode & 0x0f) == 3)	// Increment?
			fprintf(fp,     "		inc	%s\n", pu8RegPairs[(u8Opcode >> 4) & 0x03]);
		else
			fprintf(fp,     "		dec	%s\n", pu8RegPairs[(u8Opcode >> 4) & 0x03]);
...
static void LdRegReg(UINT8 u8Opcode)
{
	...
	bDestination = (UINT8) ((u8Opcode >> 3) & 0x07);
	bSource = (UINT8) ((u8Opcode) & 0x07);
...
static void MathOperationDirect(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode != 0xee && u8Opcode != 0xe6 && u8Opcode != 0xf6)
		{
			SetOverflow();
		}
		...
		if (u8Opcode == 0xc6 || u8Opcode == 0xce)
			fprintf(fp, "		and	ah, 0fdh ; Knock out N!\n");
	
		if (u8Opcode == 0xd6 || u8Opcode == 0xde || u8Opcode == 0xfe)
			fprintf(fp, "		or	ah, 02h	; Set negative!\n");

		if (u8Opcode == 0xf6 || u8Opcode == 0xee)
			fprintf(fp, "		and	ah, 0ech	; No H, N, or C\n");
	...
	if (MZ80_C == u8What)
	{
		if (0xfe == u8Opcode)	// Cp
		{
			SetSubFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "*pbPC++");
		}
		else
		if (0xe6 == u8Opcode)	// And
		{
		...
		if (0xc6 == u8Opcode) 	// Add
		{
			fprintf(fp, "				bTemp = *pbPC++;\n");
			SetAddFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
			fprintf(fp, "				cpu.z80A += bTemp;\n");
		}
		else
		if (0xce == u8Opcode) 	// Adc
		{
			fprintf(fp, "				bTemp = *pbPC++ + (cpu.z80F & Z80_FLAG_CARRY);\n");
			SetAdcFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
			fprintf(fp, "				cpu.z80A += bTemp;\n");
		}
		else
		if (0xd6 == u8Opcode) 	// Sub
		{
			fprintf(fp, "				bTemp = *pbPC++;\n");
			SetSubFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
			fprintf(fp, "				cpu.z80A -= bTemp;\n");
		}
		else
		if (0xde == u8Opcode)	// Sbc
		{
			fprintf(fp, "				bTemp = *pbPC++ + (cpu.z80F & Z80_FLAG_CARRY);\n");
			SetSbcFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
			fprintf(fp, "				cpu.z80A = cpu.z80A - bTemp;\n");
		}
		else
		if (0xee == u8Opcode)	// Xor
		{
			fprintf(fp, "				cpu.z80A ^= *pbPC++;\n");
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN);\n");
			fprintf(fp, "				cpu.z80F |= bPostORFlags[cpu.z80A];\n\n");
...
static void JrHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u88085Mode && 0x20 == u8Opcode)
		{
			// We need to do a RIM

			fprintf(fp, "		mov		al, [_8085InterruptMask]\n");
			FetchNextInstruction(u8Opcode);
			return;
		}

		if (u88085Mode && 0x30 == u8Opcode)
		{
			// We need to do a SIM

			fprintf(fp, "		mov		[_8085InterruptMask], al\n");
			FetchNextInstruction(u8Opcode);
			return;
		}
		...
		{
			fprintf(fp, "		sahf\n");
			fprintf(fp,     "		j%s	takeJump%ld\n", pbFlags[(u8Opcode >> 3) & 0x3], u32GlobalLabel);
...
static void ToRegFromHl(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (bReg != 2 && bReg != 3)
			ReadValueFromMemory((UINT8 *) "bx", (UINT8 *) pbMathReg[bReg]);
		else
		{
			ReadValueFromMemory((UINT8 *) "bx", (UINT8 *) pu8LocalReg[bReg]);
			fprintf(fp, "		mov	%s, %s\n", (UINT8 *) pbMathReg[bReg], pu8LocalReg[bReg]);
		}
	...
	if (MZ80_C == u8What)
	{
		ReadValueFromMemory((UINT8 *) "cpu.z80HL", (UINT8 *) pu8LocalRegC[bReg]);
	}
	else
	{
		assert(0);
...
static void AddRegpairOperations(UINT8 u8Opcode)
{
	...
	u8Regpair = (UINT8) ((u8Opcode >> 4) & 0x3);
...
static void PushPopOperations(UINT8 u8Opcode)
{
	...
	u8RegPair = (UINT8) (((u8Opcode >> 4) & 0x3) << 1);

	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		ProcBegin();

		if ((u8Opcode & 0xcf) == 0xc5)	// Push
		{
			fprintf(fp, "		sub	word [_z80sp], 2\n");
			fprintf(fp, "		mov	dx, [_z80sp]\n");
			WriteWordToMemory((UINT8 *) "dx", (UINT8 *) pbPopRegPairs[u8RegPair >> 1]);
		}
		else	// Pop
		{
			fprintf(fp, "		mov	dx, [_z80sp]\n");
			ReadWordFromMemory((UINT8 *) "dx", (UINT8 *) pbPopRegPairs[u8RegPair >> 1]);
	...
	if (MZ80_C == u8What)
	{
		if ((u8Opcode & 0xcf) == 0xc5)		// Push?
		{
			...
			WriteWordToMemory((UINT8 *) "cpu.z80sp", (UINT8 *) pbPopRegPairC[u8RegPair >> 1]);
			return;
		}
		else
		{
			ReadWordFromMemory((UINT8 *) "cpu.z80sp", (UINT8 *) pbPopRegPairC[u8RegPair >> 1]);
...
static void LdByteRegpair(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x0a)
			ReadValueFromMemory((UINT8 *) "cx", (UINT8 *) "al");
		if (u8Opcode == 0x1a)
		{
	...
	{
		if (u8Opcode == 0x0a)
			ReadValueFromMemory((UINT8 *) "cpu.z80BC", (UINT8 *) "cpu.z80A");
		if (u8Opcode == 0x1a)
			ReadValueFromMemory((UINT8 *) "cpu.z80DE", (UINT8 *) "cpu.z80A");
	}
	else
	{
		assert(0);
...
static void IncDecHLPtr(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		WriteValueToMemory((UINT8 *) "bx", (UINT8 *) "dl");
		fprintf(fp, "		xor	edx, edx\n");
	...
	if (MZ80_C == u8What)
	{
		...
		WriteValueToMemory((UINT8 *) "cpu.z80HL", (UINT8 *) "bTemp");
		return;
	}
	else
	{
...
static void InOutHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (0xd3 == u8Opcode)
			WriteValueToIo((UINT8 *) "dx", (UINT8 *) "al");
		else
			ReadValueFromIo((UINT8 *) "dx", (UINT8 *) "al");
	...
	if (MZ80_C == u8What)
	{
		...
		if (0xd3 == u8Opcode)
			WriteValueToIo((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80A");
		else
			ReadValueFromIo((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80A");
...
static void RESSETHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if ((u8Opcode & 0x07) == 6)     // (HL)?
			ReadValueFromMemory((UINT8 *) "bx", (UINT8 *) "dl");

		if ((u8Opcode & 0xc0) == 0x80)
			fprintf(fp, "		and %s, 0%.2xh	; Reset a bit\n",         
							pu8LocalReg[u8Op],
							0xff - (1 << ((u8Opcode >> 3) & 0x7)));

		if ((u8Opcode & 0xc0) == 0xc0)
	...
	if (MZ80_C == u8What)
	{
		...
		if ((u8Opcode & 0xc0) == 0x80)	// RES
			fprintf(fp, "				%s &= 0x%.2x;\n", pbMathRegC[u8Op], (UINT8) ~((UINT8) 1 << ((u8Opcode >> 3) & 0x07)));
		else										// SET
			fprintf(fp, "				%s |= 0x%.2x;\n", pbMathRegC[u8Op], 1 << ((u8Opcode >> 3) & 0x07));
...
static void BITHandler(UINT8 u8Opcode)
{
	...
	op = (UINT8) (u8Opcode & 0x07);
	bBitVal = (UINT8) ( 1 << ((u8Opcode >> 3) & 0x07));
...
static void RLCRRCRLRRSLASRASRLHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if ((u8Opcode & 0xf8) == 0x08)
			fprintf(fp, "		ror	%s, 1\n", pu8LocalReg[u8Op]);
		else
		if ((u8Opcode & 0xf8) == 0x10)
			fprintf(fp, "		rcl	%s, 1\n", pu8LocalReg[u8Op]);
		else
		if ((u8Opcode & 0xf8) == 0x18)
			fprintf(fp, "		rcr	%s, 1\n", pu8LocalReg[u8Op]);
		else
		if ((u8Opcode & 0xf8) == 0x20 || (u8Opcode & 0xf8) == 0x30)
			fprintf(fp, "		shl	%s, 1\n", pu8LocalReg[u8Op]);
		else
		if ((u8Opcode & 0xf8) == 0x28)
			fprintf(fp, "		sar	%s, 1\n", pu8LocalReg[u8Op]);
		else
		if ((u8Opcode & 0xf8) == 0x38)
			fprintf(fp, "		shr	%s, 1\n", pu8LocalReg[u8Op]);
		else
			assert(0);
		...
		if ((u8Opcode & 0xf8) >= 0x20)
		{
			if ((u8Opcode & 0xf8) == 0x30)
		...
		else
		{
			...
			fprintf(fp, "		or	%s, %s\n", pu8LocalReg[u8Op], pu8LocalReg[u8Op]);
	...
	if (MZ80_C == u8What)
	{
		...
		u8Opcode &= 0xf8;			// Just the instruction
		...
		if ((0x20 == u8Opcode) || (0x30 == u8Opcode))	// SLA/SRL
		{
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_ZERO | Z80_FLAG_SIGN | Z80_FLAG_HALF_CARRY | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_NEGATIVE | Z80_FLAG_CARRY);\n");
			fprintf(fp, "				cpu.z80F |= (%s >> 7);\n", pbMathRegC[u8Op]);
			fprintf(fp, "				%s = (%s << 1);\n", pbMathRegC[u8Op], pbMathRegC[u8Op]);
...
static void RRDRLDHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		ReadValueFromMemory((UINT8 *) "bx", (UINT8 *) "dl");	// Get (HL)
		fprintf(fp, "		mov	dh, dl	; Put a copy in DH\n");

		if (0x6f == u8Opcode)	// RLD
		{
...
static void CPICPDCPIRCPDRHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0xb1 || u8Opcode == 0xb9)
		{
			fprintf(fp, "cpRepeat%ld:\n", u32GlobalLabel);
			dwRepeatOb = u32GlobalLabel;
			++u32GlobalLabel;
		}
		...
		if (u8Opcode == 0xa1 || u8Opcode == 0xb1)
			fprintf(fp, "		inc	bx	; Increment!\n");
		if (u8Opcode == 0xa9 || u8Opcode == 0xb9)
	...
	if (MZ80_C == u8What)
	{
		if (0xb1 == u8Opcode || 0xb9 == u8Opcode)	// Repeat instruction?
		{
		...
		if (0xb1 == u8Opcode || 0xa1 == u8Opcode)
		{
		...
		if (0xb1 == u8Opcode || 0xb9 == u8Opcode)	// Repeat?
		{
...
static void INIRINDRINIINDHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		dwTempLabel = u32GlobalLabel;
		u32GlobalLabel++;

		if (0xba == u8Opcode || 0xb2 == u8Opcode)
		...
		ReadValueFromIo((UINT8 *) "cx", (UINT8 *) "*dl");		// Put our value in DL
		fprintf(fp, "		pop	cx	; Restore BC\n");
	
		WriteValueToMemory((UINT8 *) "bx", (UINT8 *) "dl");
	
		if (0xa2 == u8Opcode || 0xb2 == u8Opcode)
			fprintf(fp, "		inc	bx	; Increment HL\n");
		else
		if (0xaa == u8Opcode || 0xba == u8Opcode)
	...
	if (MZ80_C == u8What)
	{
		if (0xb2 == u8Opcode || 0xba == u8Opcode)	// Repeat instruction?
		{
		...
		ReadValueFromIo((UINT8 *) "cpu.z80B", (UINT8 *) "bTemp");
		WriteValueToMemory((UINT8 *) "cpu.z80HL", (UINT8 *) "bTemp");

		if (0xb2 == u8Opcode || 0xa2 == u8Opcode)
		{
			fprintf(fp, "				cpu.z80HL++;\n");
			fprintf(fp, "				cpu.z80HL &= 0xffff;\n");
		}
...
static void OTIROTDROUTIOUTDHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (0xbb == u8Opcode || 0xb3 == u8Opcode)
			fprintf(fp, "loopIt%ld:\n", dwTempLabel);
		...
		WriteValueToIo((UINT8 *) "cx", (UINT8 *) "dl");
		fprintf(fp, "		pop	cx	; Restore BC now that it has been \"OUT\"ed\n");
	
		if (0xa3 == u8Opcode || 0xb3 == u8Opcode)
			fprintf(fp, "		inc	bx	; Increment HL\n");
		else
		if (0xab == u8Opcode || 0xbb == u8Opcode)
			fprintf(fp, "		dec	bx	; Decrement HL\n");
	...
	if (MZ80_C == u8What)
	{
		if (0xb3 == u8Opcode || 0xbb == u8Opcode)	// Repeat instruction?
		{
			fprintf(fp, "				while ((sdwCyclesRemaining > 0) && (cpu.z80B))\n");
		}
		...
		ReadValueFromMemory((UINT8 *) "cpu.z80HL", (UINT8 *) "bTemp");
		WriteValueToIo((UINT8 *) "cpu.z80BC", (UINT8 *) "bTemp");

		if (0xb3 == u8Opcode || 0xa3 == u8Opcode)
		{
			fprintf(fp, "				cpu.z80HL++;\n");
			fprintf(fp, "				cpu.z80HL &= 0xffff;\n");
		}
...
static void ExtendedOutHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		WriteValueToIo((UINT8 *) "dx", (UINT8 *) pbMathReg[(u8Opcode >> 3) & 0x07]);
	...
	if (MZ80_C == u8What)
	{
		...
		WriteValueToIo((UINT8 *) "dwAddr", (UINT8 *) pbMathRegC[(u8Opcode >> 3) & 0x07]);
	}
	else
		assert(0);
}
...
static void ExtendedInHandler(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		ReadValueFromIo((UINT8 *) "dx", (UINT8 *) pbMathReg[(u8Opcode >> 3) & 0x07]);

		fprintf(fp, ";\n; Remember, this variant of the IN instruction modifies the flags\n;\n\n");
		fprintf(fp, "		sahf	; Restore our flags\n");
		fprintf(fp, "		mov	dh, ah	; Save flags for later\n");
	
		if (0x50 == u8Opcode || 0x58 == u8Opcode)
		{
			fprintf(fp, "		mov	dl, %s\n", pbMathReg[(u8Opcode >> 3) & 0x07]);
			fprintf(fp, "		or	dl, dl\n");
		}
		else
			fprintf(fp, "		or	%s, %s;\n", pbMathReg[(u8Opcode >> 3) & 0x07], pbMathReg[(u8Opcode >> 3) & 0x07]);
	...
	if (MZ80_C == u8What)
	{
		...
		ReadValueFromIo((UINT8 *) "dwAddr", (UINT8 *) pbMathRegC[(u8Opcode >> 3) & 0x07]);

		// Set flags!

		fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_SIGN | Z80_FLAG_ZERO | Z80_FLAG_HALF_CARRY | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_NEGATIVE);\n");
		fprintf(fp, "				cpu.z80F |= bPostORFlags[%s];\n", pbMathRegC[(u8Opcode >> 3) & 0x07]);
	}
	else
		assert(0);
}
...
static void NegHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_C == u8What)
	{
		SetSubFlagsSZHVC((UINT8 *) "0", (UINT8 *) "cpu.z80A");
...
static void ExtendedRegIntoMemory(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x43)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "cl");
		if (u8Opcode == 0x53)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80de]");
		if (u8Opcode == 0x63)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "bl");
		if (u8Opcode == 0x73)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80sp]");

		fprintf(fp, "		inc	dx\n");

		if (u8Opcode == 0x43)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "ch");
		if (u8Opcode == 0x53)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80de + 1]");
		if (u8Opcode == 0x63)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "bh");
		if (u8Opcode == 0x73)
			WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80sp + 1]");
	...
	if (MZ80_C == u8What)
	{
		...
		if (0x43 == u8Opcode)		//	LD (xxxxh), BC
			WriteWordToMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80BC");
		if (0x53 == u8Opcode)		//	LD (xxxxh), DE
			WriteWordToMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80DE");
		if (0x63 == u8Opcode)		//	LD (xxxxh), HL
			WriteWordToMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80HL");
		if (0x73 == u8Opcode)		//	LD (xxxxh), SP
			WriteWordToMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80sp");
	}
	else
		assert(0);
}
...
static void LdRegpair(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x4b)
			ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "cl");
		if (u8Opcode == 0x5b)
			ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80de]");
		if (u8Opcode == 0x7b)
			ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80sp]");
	
		fprintf(fp, "		inc	dx\n");
	
		if (u8Opcode == 0x4b)
			ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "ch");
		if (u8Opcode == 0x5b)
			ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80de + 1]");
		if (u8Opcode == 0x7b)
			ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "byte [_z80sp + 1]");
	...
	if (MZ80_C == u8What)
	{
		...
		if (0x4b == u8Opcode)
			ReadWordFromMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80BC");
		if (0x5b == u8Opcode)
			ReadWordFromMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80DE");
		if (0x7b == u8Opcode)
			ReadWordFromMemory((UINT8 *) "dwTemp", (UINT8 *) "cpu.z80sp");
	}
	else
		assert(0);
}
...
static void LDILDRLDIRLDDRHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0xb0 || u8Opcode == 0xb8)
		{
			dwOrgGlobal = u32GlobalLabel;
		...
		WriteValueToMemory((UINT8 *) "[_z80de]", (UINT8 *) "dl");
		...
		if (u8Opcode == 0xa0 || u8Opcode == 0xa8)
		{
			fprintf(fp, "		or	ah, 04h	; Non-zero - we're still going!\n");
		}
	...
	if (MZ80_C == u8What)
	{
		...
		if (0xb0 == u8Opcode || 0xb8 == u8Opcode)	// Repeat instruction?
		{
			fprintf(fp, "				while ((sdwCyclesRemaining > 0) && (cpu.z80BC))\n");
		...
		ReadValueFromMemory((UINT8 *) "cpu.z80HL", (UINT8 *) "bTemp");
		WriteValueToMemory((UINT8 *) "cpu.z80DE", (UINT8 *) "bTemp");
		...
		if (0xb0 == u8Opcode || 0xb8 == u8Opcode)
		{
...
static void IRHandler(UINT8 u8Opcode)
{
   char *src, *dst;
	...
	if (MZ80_C == u8What)
	{
		...
		if (0x47 == u8Opcode)		// LD I, A
		{
		 	fprintf(fp, "				cpu.z80i = cpu.z80A;\n");
		}
		else
		if (0x57 == u8Opcode)		// LD A, I
		{
...
static void DDFDCBHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_C == u8What)
	{
		if (strcmp((const char *) "cpu.z80IX", u8mz80Index) == 0)
			s32Data = 0;
		else
			s32Data = 1;
...
static void LoadIndexReg(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf((char *) string, "[_z80%s]", (char *) u8mz80Index);

		fprintf(fp, "		mov	dx, [esi]	 ; Get our address to store\n");
		fprintf(fp, "		add	esi, 2\n");

		ReadWordFromMemory((UINT8 *) "dx", string);
		fprintf(fp, "		xor	edx, edx\n");
		FetchNextInstruction(u8Opcode);
	}
	else
	if (MZ80_C == u8What)
	{
		fprintf(fp, "				dwAddr = *pbPC++;\n");
		fprintf(fp, "				dwAddr |= ((UINT32) *pbPC++ << 8);\n");
		ReadWordFromMemory((UINT8 *) "dwAddr", (UINT8 *) u8mz80Index);
	}
	else
		assert(0);
}
...
static void StoreIndexReg(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		WriteValueToMemory((UINT8 *) "[_orgval]", (UINT8 *) "dl");
	...
	if (MZ80_C == u8What)
	{
		fprintf(fp, "				dwAddr = *pbPC++;\n");
		fprintf(fp, "				dwAddr |= ((UINT32) *pbPC++ << 8);\n");
		WriteWordToMemory((UINT8 *) "dwAddr", (UINT8 *) u8mz80Index);
	}
	else
		assert(0);
}
...
static void LdIndexPtrReg(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		IndexedOffset((UINT8 *) u8mz80Index);

		// DX Contains the address

		WriteValueToMemory((UINT8 *) "dx", (UINT8 *) pbMathReg[u8Opcode & 0x07]);
		fprintf(fp, "		xor	edx, edx\n");
	...
	if (MZ80_C == u8What)
	{
		...
		WriteValueToMemory((UINT8 *) "sdwAddr", (UINT8 *) pbMathRegC[u8Opcode & 0x07]);
	}
	else
		assert(0);
}
...
static void UndocMathIndex(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		u8Opcode1 = (UINT8) (u8Opcode & 0xf8);	// Only the operation
		...
		if (u8Opcode1 != 0xa8 && u8Opcode1 != 0xa0 && u8Opcode1 != 0xb0)
		{
			SetOverflow();
		}
		...
		if (u8Opcode1 == 0x80 || u8Opcode1 == 0x88)
			fprintf(fp, "		and	ah, 0fdh ; No N!\n");
	
		if (u8Opcode1 == 0x90 || u8Opcode1 == 0x98)
			fprintf(fp, "		or	ah, 02h	; N Gets set!\n");
	...
	{
		if (u8Opcode & 1)
			pbIndexReg = (UINT8 *) u8mz80IndexHalfLow;
		else
			pbIndexReg = (UINT8 *) u8mz80IndexHalfHigh;

		u8Opcode1 = (UINT8) (u8Opcode & 0xf8);	// Only the operation

		if (0x80 == u8Opcode1)	// ADD
		{
			fprintf(fp, "				bTemp2 = cpu.z80A + %s;\n", pbIndexReg);
			SetAddFlagsSZHVC((UINT8 *) "cpu.z80A", pbIndexReg);
		}
		else
		if (0x88 == u8Opcode1)	// ADC
		{
			fprintf(fp, "				bTemp2 = cpu.z80A + %s + (cpu.z80F & Z80_FLAG_CARRY);\n", pbIndexReg);
			SetAdcFlagsSZHVC((UINT8 *) "cpu.z80A", pbIndexReg);
		}
		else
		if (0x90 == u8Opcode1)	// SUB
		{
			fprintf(fp, "				bTemp2 = cpu.z80A - %s;\n", pbIndexReg);
			SetSubFlagsSZHVC((UINT8 *) "cpu.z80A", pbIndexReg);
		}
		else								
		if (0x98 == u8Opcode1)	// SBC
		{
			fprintf(fp, "				bTemp2 = cpu.z80A - %s - (cpu.z80F & Z80_FLAG_CARRY);\n", pbIndexReg);
			SetSbcFlagsSZHVC((UINT8 *) "cpu.z80A", pbIndexReg);
		}
		else
		if (0xa0 == u8Opcode1)	// AND
		{
			fprintf(fp, "				cpu.z80A &= %s;\n", pbIndexReg);
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN);\n");
			fprintf(fp, "				cpu.z80F |= bPostANDFlags[cpu.z80A];\n\n");
		...
		if (0xb8 == u8Opcode1)	// CP - Don't do anything! Just flags!
		{
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN);\n");
			fprintf(fp, "				cpu.z80F |= bPostORFlags[cpu.z80A];\n\n");
		}
...
static void UndocLoadHalfIndexReg(UINT8 u8Opcode)
{
	...
	{
		if (u8Opcode & 0x08)
			fprintf(fp, "			%s = *pbPC++;\n", (char *) u8mz80IndexHalfLow);
		else
			fprintf(fp, "			%s = *pbPC++;\n", (char *) u8mz80IndexHalfHigh);
	}
	else
		assert(0);
}
...
static void UndocIncDecIndexReg(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if ((0x24 == u8Opcode) || (0x2c == u8Opcode))
			fprintf(fp, "		and	ah, 0fdh	; Knock out N!\n");
		else
			fprintf(fp, "		or	ah, 02h	; Set negative!\n");
...
static void ExIndexed(UINT8 u8Opcode)
{
	...
	{
		ReadWordFromMemory((UINT8 *) "cpu.z80sp", (UINT8 *) "dwAddr");
		WriteWordToMemory((UINT8 *) "cpu.z80sp", (UINT8 *) u8mz80Index);
		fprintf(fp, "				%s = dwAddr;\n", (char *) u8mz80Index);
	}
	else
		assert(0);
...
static void IncDecIndexReg(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x23)
			fprintf(fp, "		inc	word [_z80%s]	; Increment our u8mz80Index register\n", (char *) u8mz80Index);
		else
			fprintf(fp, "		dec	word [_z80%s]	; Increment our (char *) u8mz80Index register\n", (char *) u8mz80Index);
...
static void LdRegIndexOffset(UINT8 u8Opcode)
{
	...
	u8Opcode1 = (UINT8) ((u8Opcode & 0x38) >> 3);
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) pbMathReg[u8Opcode1]);
	...
	if (MZ80_C == u8What)
	{
		...
		ReadValueFromMemory((UINT8 *) "sdwAddr", (UINT8 *) pbMathRegC[u8Opcode1]);
	}
	else
		assert(0);
}
...
static void LdByteToIndex(UINT8 u8Opcode)
{
	...
	if (MZ80_C == u8What)
	{
		...
		WriteValueToMemory((UINT8 *) "sdwAddr", (UINT8 *) "*pbPC++");
	}
	else
		assert(0);
}
...
static void AddIndexHandler(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		fprintf(fp, "		add	di, %s\n", pbIndexedRegPairs[(u8Opcode & 0x30) >> 4]);
		fprintf(fp, "		lahf\n");
	...
	if (MZ80_C == u8What)
	{
		...
		{ 
			fprintf(fp, "			cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_HALF_CARRY);\n");
			fprintf(fp, "			dwTemp = %s + %s;\n", (char *) u8mz80Index, pu8RegPairsC[u8RegPair]);
		...
		{
			fprintf(fp, "			cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_HALF_CARRY);\n");
			fprintf(fp, "			dwTemp = %s + %s;\n", (char *) u8mz80Index, (char *) u8mz80Index);
			fprintf(fp, "			cpu.z80F |= ((dwTemp >> 16) & Z80_FLAG_CARRY) | (((%s ^ dwTemp ^ %s) >> 8) & Z80_FLAG_HALF_CARRY);\n", (char *) u8mz80Index, pu8RegPairsC[u8RegPair]);
			fprintf(fp, "			%s = dwTemp & 0xffff;\n", (char *) u8mz80Index);
		}
	}
...
static void IncDecIndexed(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		ReadValueFromMemory((UINT8 *) "dx", (UINT8 *) "dl");
	...
	if (MZ80_C == u8What)
	{
		...
		ReadValueFromMemory((UINT8 *) "dwAddr", (UINT8 *) "bTemp");
		...
		WriteValueToMemory((UINT8 *) "dwAddr", (UINT8 *) "bTemp");
	}
	else
		assert(0);
}
...
static void MathOperationIndexed(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if (u8Opcode == 0x86)           // Add
			fprintf(fp, "		add	al, dl\n");
		if (u8Opcode == 0x8e)           // Adc
			fprintf(fp, "		adc	al, dl\n");
		if (u8Opcode == 0x96)           // Sub
			fprintf(fp, "		sub	al, dl\n");
		if (u8Opcode == 0x9e)           // Sbc
			fprintf(fp, "		sbb	al, dl\n");
		if (u8Opcode == 0xa6)           // And
			fprintf(fp, "		and	al, dl\n");
		if (u8Opcode == 0xae)           // Xor
			fprintf(fp, "		xor	al, dl\n");
		if (u8Opcode == 0xb6)           //      Or
			fprintf(fp, "		or	al, dl\n");
		if (u8Opcode == 0xbe)           // Cp
		...
		if (u8Opcode == 0x86 || u8Opcode == 0x8e)
		{
			SetOverflow();
			fprintf(fp, "		and	ah, 0fdh	; Knock out negative\n");
		}

		if (u8Opcode == 0x96 || u8Opcode == 0x9e || u8Opcode == 0xbe)
		{
			SetOverflow();
			fprintf(fp, "		or	ah, 02h	; Set negative\n");
		}

		if (u8Opcode == 0xae || u8Opcode == 0xb6)
	...
	if (MZ80_C == u8What)
	{
		...
		if (0x86 == u8Opcode)		// ADD A, (IX/IY+nn)
		{
			SetAddFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
			fprintf(fp, "				cpu.z80A += bTemp;\n");
		}
		else
		if (0x8e == u8Opcode)		// ADC A, (IX/IY+nn)
		{
			fprintf(fp, "				bTemp2 = (cpu.z80F & Z80_FLAG_CARRY);\n");
			SetAdcFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
		...
		if (0x96 == u8Opcode)		// SUB A, (IX/IY+nn)
		{
			SetSubFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
			fprintf(fp, "				cpu.z80A -= bTemp;\n");
		}
		else
		if (0x9e == u8Opcode)		// SBC A, (IX/IY+nn)
		{
			fprintf(fp, "				bTemp2 = cpu.z80A;\n");
			fprintf(fp, "				cpu.z80A = cpu.z80A - bTemp - (cpu.z80F & Z80_FLAG_CARRY);\n");
			SetSbcFlagsSZHVC((UINT8 *) "bTemp2", (UINT8 *) "bTemp");
		}
		else
		if (0xa6 == u8Opcode)		// AND A, (IX/IY+nn)
		{
			fprintf(fp, "				cpu.z80A &= bTemp;\n");
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN);\n");
			fprintf(fp, "				cpu.z80F |= bPostANDFlags[cpu.z80A];\n\n");
		...
		if (0xae == u8Opcode)		// XOR A, (IX/IY+nn)
		{
			fprintf(fp, "				cpu.z80A ^= bTemp;\n");
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN);\n");
			fprintf(fp, "				cpu.z80F |= bPostORFlags[cpu.z80A];\n\n");
		...
		if (0xb6 == u8Opcode)		// OR A, (IX/IY+nn)
		{
			fprintf(fp, "				cpu.z80A |= bTemp;\n");
			fprintf(fp, "				cpu.z80F &= ~(Z80_FLAG_CARRY | Z80_FLAG_NEGATIVE | Z80_FLAG_OVERFLOW_PARITY | Z80_FLAG_HALF_CARRY | Z80_FLAG_ZERO | Z80_FLAG_SIGN);\n");
			fprintf(fp, "				cpu.z80F |= bPostORFlags[cpu.z80A];\n\n");
		...
		if (0xbe == u8Opcode)		// CP A, (IX/IY+nn)
		{
			SetSubFlagsSZHVC((UINT8 *) "cpu.z80A", (UINT8 *) "bTemp");
		}
		else
...
static void UndocIndexToReg(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if ((u8Opcode & 0x07) == 2 || (u8Opcode & 0x07) == 3)
		...
		fprintf(fp, "		mov   byte [_z80%s + %ld], %s\n", (char *) u8mz80Index, (long int) (1 - ((u8Opcode & 0x08) >> 3)), pu8LocalReg[u8Opcode & 0x07]);
		fprintf(fp, "		xor	edx, edx\n");
		FetchNextInstruction(u8Opcode);
	}
	else
	if (MZ80_C == u8What)
	{
		if (u8Opcode != 0x64 && u8Opcode != 0x65 && u8Opcode != 0x6c && u8Opcode != 0x6d)
		{
			if (u8Opcode & 0x08)
				fprintf(fp, "			%s = %s;\n", (char *) u8mz80IndexHalfLow, pu8LocalRegC[u8Opcode & 0x07]);
			else
				fprintf(fp, "			%s = %s;\n", (char *) u8mz80IndexHalfHigh, pu8LocalRegC[u8Opcode & 0x07]);
		}
		else		// IX/IY High/low weirdness afoot...
		{
			// We don't generate any code for ld indexH, indexH and ld indexL, indexL

			if (0x65 == u8Opcode)		// LD indexH, indexL
			{
				fprintf(fp, "			%s = %s;\n", (char *) u8mz80IndexHalfHigh, (char *) u8mz80IndexHalfLow);
			}
			else
			if (0x6c == u8Opcode)		// LD indexH, indexL
			{
				fprintf(fp, "			%s = %s;\n", (char *) u8mz80IndexHalfLow, (char *) u8mz80IndexHalfHigh);
			}
		}
	}
	else
...
static void UndocRegToIndex(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		if ((u8Opcode & 0x38) == 0x10 || (u8Opcode & 0x38) == 0x18)
			fprintf(fp, "		mov	dx, [_z80de]	; Get a usable copy of DE here\n");

		fprintf(fp, "		mov	%s, byte [_z80%s + %ld]\n", pu8LocalReg[(u8Opcode >> 3) & 0x07], (char *) u8mz80Index, (long int) (1 - (u8Opcode & 1)));
	...
	{
		if (u8Opcode & 1)
			fprintf(fp, "			%s = %s;\n", pu8LocalRegC[(u8Opcode >> 3) & 0x07], (char *) u8mz80IndexHalfLow);
		else
			fprintf(fp, "			%s = %s;\n", pu8LocalRegC[(u8Opcode >> 3) & 0x07], (char *) u8mz80IndexHalfHigh);
	}
	else
		assert(0);
}
...
static void PushPopOperationsIndexed(UINT8 u8Opcode)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		sprintf((char *) bRegBaseLsb, "byte [_z80%s]", (char *) u8mz80Index);
		sprintf((char *) bRegBaseMsb, "byte [_z80%s + 1]", (char *) u8mz80Index);

		sprintf((char *) string, "[_z80%s]", (char *) u8mz80Index);

		ProcBegin();

		if (u8Opcode == 0xe5)	// Push IX/IY
		{
			fprintf(fp, "		sub	word [_z80sp], 2\n");
			fprintf(fp, "		mov	dx, [_z80sp]\n");
	
			WriteWordToMemory((UINT8 *) "dx", string);		
		}
		else	// Pop
		{
			fprintf(fp, "		mov	dx, [_z80sp]\n");
	...
	if (MZ80_C == u8What)
	{
		...
		if (0xe1 == u8Opcode)	// Pop IX/IY
		{
			ReadWordFromMemory((UINT8 *) "cpu.z80sp", (UINT8 *) u8mz80Index);
...
static void ddcbBitWise(UINT8 u8Opcode)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		// already computed plus the u8mz80Index register.
		...
		if (u8Opcode != 0x06 && u8Opcode != 0x0e &&
			 u8Opcode != 0x16 && u8Opcode != 0x1e &&
			 u8Opcode != 0x26 && u8Opcode != 0x2e &&
			 u8Opcode != 0x3e && (u8Opcode & 0xc7) != 0x86 &&
			 (u8Opcode & 0xc7) != 0xc6)
		{
		...
		if ((u8Opcode & 0xc7) != 0x86 && (u8Opcode & 0xc7) != 0xc6)
			fprintf(fp, "		sahf		; Restore our flags\n");
		...
		if ((u8Opcode & 0xc7) == 0x46)
			fprintf(fp, "		test	dl, 0%.2xh	; Is it set?\n", (1 << ((u8Opcode >> 3) & 0x07)));
		else
		if ((u8Opcode & 0xc7) == 0x86)
			fprintf(fp, "		and	dl, 0%.2xh	; Reset the bit\n", 
						0xff - (1 << ((u8Opcode >> 3) & 0x07)));
		else
		if ((u8Opcode & 0xc7) == 0xc6)
		...
		if (u8Opcode == 0x26 || u8Opcode == 0x2e || ((u8Opcode & 0xc7) == 0x46))
		...
		if ((u8Opcode & 0xc7) != 0x46)  // If it's not a BIT, write it back
			WriteValueToMemory((UINT8 *) "[_orgval]", (UINT8 *) "dl");
	...
	if (MZ80_C == u8What)
	{
		...
		if ((u8Opcode & 0xc0) == 0x80)	// RES
		{
			fprintf(fp, "				bTemp &= 0x%.2x;\n", ~(1 << ((u8Opcode >> 3) & 0x07)) & 0xff);
			WriteValueToMemory((UINT8 *) "dwAddr", (UINT8 *) "bTemp");
		}
		else
		if ((u8Opcode & 0xc0) == 0xC0)	// SET
		{
			fprintf(fp, "				bTemp |= 0x%.2x;\n", 1 << ((u8Opcode >> 3) & 0x07));
			WriteValueToMemory((UINT8 *) "dwAddr", (UINT8 *) "bTemp");
		}
...
static void GetTicksCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		Alignment();
		sprintf(u8ProcName, "%sGetElapsedTicks_", u8CpuBaseName);
		ProcBegin();
...
static void ReleaseTimesliceCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		Alignment();
		sprintf(u8ProcName, "%sReleaseTimeslice_", u8CpuBaseName);
		ProcBegin();
...
static void DataSegment(void)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		fprintf(fp, "_8085InterruptMask	db	0	; Only for 8085 - interrupt mask\n");
		fprintf(fp, "\n");
		...
		for (u32Loop = 0; u32Loop < 256; u32Loop++)
			bUsed[u32Loop] = 0;
	...
	// Now rip through and find out what is and isn't used
		...
		while (StandardOps[u32Loop].Emitter)
		{
			assert(StandardOps[u32Loop].u8OpCode < 0x100);
			if (bUsed[StandardOps[u32Loop].u8OpCode])
			{
				fprintf(stderr, "Oops! %.2x\n", (unsigned int) u32Loop);
				fclose(fp);
				exit(1);
			}
			bUsed[StandardOps[u32Loop].u8OpCode] = 1;
			u32Loop++;
		}
		...
		while (u32Loop < 0x100)
		{
			fprintf(fp, "		dd	");
			if (bUsed[u32Loop])
				fprintf(fp, "RegInst%.2x", (unsigned int) u32Loop);
			else
		...
		{		
			...
			while (CBOps[u32Loop].Emitter)
			{
				assert(CBOps[u32Loop].u8OpCode < 0x100);
				if (bUsed[CBOps[u32Loop].u8OpCode])
				{
					fprintf(stderr, "Oops CB! %.2x\n", (unsigned int) u32Loop);
					fclose(fp);
					exit(1);
				}
				bUsed[CBOps[u32Loop].u8OpCode] = 1;
				u32Loop++;
			}
			...
			while (u32Loop < 0x100)
			{
				fprintf(fp, "		dd	");
				if (bUsed[u32Loop])
					fprintf(fp, "CBInst%.2x", (unsigned int) u32Loop);
				else
			...
			while (EDOps[u32Loop].Emitter)
			{
				assert(EDOps[u32Loop].u8OpCode < 0x100);
				if (bUsed[EDOps[u32Loop].u8OpCode])
				{
					fprintf(stderr, "Oops ED! %.2x\n", (unsigned int) u32Loop);
					fclose(fp);
					exit(1);
				}
				bUsed[EDOps[u32Loop].u8OpCode] = 1;
				u32Loop++;
			}
			...
			while (u32Loop < 0x100)
			{
				fprintf(fp, "		dd	");
				if (bUsed[u32Loop])
					fprintf(fp, "EDInst%.2x", (unsigned int) u32Loop);
				else
					fprintf(fp, "invalidInsWord");
				fprintf(fp, "\n");
				u32Loop++;
			...
			while (DDFDOps[u32Loop].Emitter)
			{
				assert(DDFDOps[u32Loop].u8OpCode < 0x100);
				if (bUsed[DDFDOps[u32Loop].u8OpCode])
				{
					fprintf(stderr, "Oops DD! %.2x\n", bUsed[DDFDOps[u32Loop].u8OpCode]);
					fclose(fp);
					exit(1);
				}
				bUsed[DDFDOps[u32Loop].u8OpCode] = 1;
				u32Loop++;
			}
			...
			while (u32Loop < 0x100)
			{
				fprintf(fp, "		dd	");
				if (bUsed[u32Loop])
					fprintf(fp, "DDInst%.2x", (unsigned int) u32Loop);
				else
					fprintf(fp, "invalidInsWord");
				fprintf(fp, "\n");
				u32Loop++;
		...
		// Now rip through and find out what is and isn't used (FD Ops)
			...
			while (u32Loop < 0x100)
			{
				fprintf(fp, "		dd	");
				if (bUsed[u32Loop])
					fprintf(fp, "FDInst%.2x", (unsigned int) u32Loop);
				else
					fprintf(fp, "invalidInsWord");
				fprintf(fp, "\n");
				u32Loop++;
			...
			while (DDFDCBOps[u32Loop].Emitter)
			{
				assert(DDFDCBOps[u32Loop].u8OpCode < 0x100);
				if (bUsed[DDFDCBOps[u32Loop].u8OpCode])
				{
					fprintf(stderr, "Oops CBFDDD! %.2x\n", bUsed[DDFDCBOps[u32Loop].u8OpCode]);
					fclose(fp);
					exit(1);
				}
				bUsed[DDFDCBOps[u32Loop].u8OpCode] = 1;
				u32Loop++;
			}
			...
			while (u32Loop < 0x100)
			{
				fprintf(fp, "		dd	");
				if (bUsed[u32Loop])
					fprintf(fp, "DDFDCBInst%.2x", (unsigned int) u32Loop);
				else
					fprintf(fp, "invalidInsWord");
				fprintf(fp, "\n");
				u32Loop++;
	...
	if (MZ80_C == u8What)
	{
		...
		if (u8StaticFlagTable)
		{
			...
			fprintf(fp, "static const pbAddAdcTable[256*256*2] = \n");
			fprintf(fp, "{\n	");
			...
			fprintf(fp, "static const pbSubSbcTable[256*256*2] = \n");
			fprintf(fp, "{\n");
...
static void CodeSegmentBegin(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
...
static void EmitRegularInstructions(void)
{
	UINT32 u32Loop = 0;
	UINT32 u32Loop2 = 0;

	u8CurrentMode = TIMING_REGULAR;
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		{
			u32Loop2 = 0;
			sprintf(u8ProcName, "RegInst%.2x", (unsigned int) u32Loop);

			while (StandardOps[u32Loop2].u8OpCode != u32Loop && StandardOps[u32Loop2].u8OpCode != 0xffffffff)
				u32Loop2++;

			assert(u32Loop2 < 0x100);
			if (StandardOps[u32Loop2].Emitter
				&& StandardOps[u32Loop2].u8OpCode != 0xffffffff)
				StandardOps[u32Loop2].Emitter((UINT8) u32Loop);
	...
	if (MZ80_C == u8What)
	{
		...
		fprintf(fp, "	{\n");
		fprintf(fp, "		u8OpCode = *pbPC++;\n");
		fprintf(fp, "		switch (u8OpCode)\n");
		fprintf(fp, "		{\n");
		...
		while (u32Loop < 0x100)
		{
			...
			if (u8TimingRegular[u32Loop])
			{
				fprintf(fp, "				sdwCyclesRemaining -= %ld;\n", (long int) u8TimingRegular[u32Loop]);
			}
...
static void EmitCBInstructions(void)
{
	UINT32 u32Loop = 0;
	UINT32 u32Loop2 = 0;

	u8CurrentMode = TIMING_CB;

	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		while (u32Loop < 0x100)
		{
			sprintf(u8ProcName, "CBInst%.2x", (unsigned int) u32Loop);
			u32Loop2 = 0;

			while (CBOps[u32Loop2].u8OpCode != u32Loop && CBOps[u32Loop2].u8OpCode != 0xffffffff)
				u32Loop2++;

			assert(u32Loop2 < 0x100);
			if (CBOps[u32Loop2].Emitter && CBOps[u32Loop2].u8OpCode != 0xffffffff)
				CBOps[u32Loop2].Emitter((UINT8) u32Loop);
	...
	if (MZ80_C == u8What)
	{
		...
		while (u32Loop < 0x100)
		{
			...
			if (u8TimingCB[u32Loop])
			{
				fprintf(fp, "			sdwCyclesRemaining -= %ld;\n", u8TimingCB[u32Loop]);
			}

			if (CBOps[u32Loop2].Emitter)
			{
				CBOps[u32Loop2].Emitter((UINT8) u32Loop);
			}
			else
...
static void EmitEDInstructions(void)
{
	UINT32 u32Loop = 0;
	UINT32 u32Loop2 = 0;

	u8CurrentMode = TIMING_ED;

	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		while (u32Loop < 0x100)
		{
			sprintf(u8ProcName, "EDInst%.2x", u32Loop);
			u32Loop2 = 0;

			while (EDOps[u32Loop2].u8OpCode != u32Loop && EDOps[u32Loop2].u8OpCode != 0xffffffff)
				u32Loop2++;

			assert(u32Loop2 < 0x100);
			if (EDOps[u32Loop2].Emitter && EDOps[u32Loop2].u8OpCode != 0xffffffff)
				EDOps[u32Loop2].Emitter((UINT8) u32Loop);
	...
	if (MZ80_C == u8What)
	{
		...
		while (u32Loop < 0x100)
		{
			...
			if (u8TimingED[u32Loop])
			{
				fprintf(fp, "			sdwCyclesRemaining -= %ld;\n", u8TimingED[u32Loop]);
			}

			if (EDOps[u32Loop2].Emitter)
			{
				EDOps[u32Loop2].Emitter((UINT8) u32Loop);
			}
			else
...
static void EmitDDInstructions(void)
{
	UINT32 u32Loop = 0;
	UINT32 u32Loop2 = 0;

	u8CurrentMode = TIMING_DDFD;

	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		while (u32Loop < 0x100)
		{
			sprintf(u8ProcName, "DDInst%.2x", u32Loop);
			u32Loop2 = 0;
	
			while (DDFDOps[u32Loop2].u8OpCode != u32Loop && DDFDOps[u32Loop2].u8OpCode != 0xffffffff)
				u32Loop2++;
	
			assert(u32Loop2 < 0x100);
			if (DDFDOps[u32Loop2].Emitter && DDFDOps[u32Loop2].u8OpCode != 0xffffffff)
				DDFDOps[u32Loop2].Emitter((UINT8) u32Loop);
	
			u32Loop++;
		}

		u8CurrentMode = TIMING_XXCB;

		u32Loop = 0;

		while (u32Loop < 0x100)
		{
			sprintf(u8ProcName, "DDFDCBInst%.2x", u32Loop);
			u32Loop2 = 0;

			while (DDFDCBOps[u32Loop2].u8OpCode != u32Loop && DDFDCBOps[u32Loop2].u8OpCode != 0xffffffff)
				u32Loop2++;

			assert(u32Loop2 < 0x100);
			if (DDFDCBOps[u32Loop2].Emitter && DDFDCBOps[u32Loop2].u8OpCode != 0xffffffff)
				DDFDCBOps[u32Loop2].Emitter((UINT8) u32Loop);
	...
	if (MZ80_C == u8What)
	{
		...
		while (u32Loop < 0x100)
		{
			...
			if (u8TimingDDFD[u32Loop])
			{
				fprintf(fp, "			sdwCyclesRemaining -= %ld;\n", u8TimingDDFD[u32Loop]);
			}

			if (DDFDOps[u32Loop2].Emitter)
			{
				DDFDOps[u32Loop2].Emitter((UINT8) u32Loop);
			}
			else
		...
		while (u32Loop < 0x100)
		{
			...
			if (u8TimingXXCB[u32Loop])
			{
				fprintf(fp, "			sdwCyclesRemaining -= %ld;\n", u8TimingXXCB[u32Loop]);
			}

			if (DDFDCBOps[u32Loop2].Emitter)
			{
				DDFDCBOps[u32Loop2].Emitter((UINT8) u32Loop);
			}
			else
...
static void EmitFDInstructions(void)
{
	UINT32 u32Loop = 0;
	UINT32 u32Loop2 = 0;
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		while (u32Loop < 0x100)
		{
			sprintf(u8ProcName, "FDInst%.2x", u32Loop);
			u32Loop2 = 0;
...
static void WriteMemoryByteHandler(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		Alignment();
...
static void PushWordHandler(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		Alignment();
		...
		fprintf(fp, "		mov	dx, [_z80sp]\n");
		fprintf(fp, "		dec	dx\n");
		WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "byte [_wordval+1]");
		fprintf(fp, "		dec	dx\n");
		WriteValueToMemory((UINT8 *) "dx", (UINT8 *) "byte [_wordval]");
...
static void PopWordHandler(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		Alignment();
		...
		ReadWordFromMemory((UINT8 *) "dx", (UINT8 *) "dx");
		fprintf(fp, "		ret\n\n");
	}
}

static void ReadIoHandler(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		Alignment();
...
static void WriteIoHandler(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		Alignment();
...
static void ExecCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sexec_", u8CpuBaseName);
		ProcBegin();
...
static void NmiCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%snmi_", u8CpuBaseName);
		ProcBegin();
		fprintf(fp, "_%snmi:\n", u8CpuBaseName);
...
static void IntCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sint_", u8CpuBaseName);
		ProcBegin();
		fprintf(fp, "_%sint:\n", u8CpuBaseName);
		...
		sprintf(u8ProcName, "%sClearPendingInterrupt_", u8CpuBaseName);
		ProcBegin();
...
static void ResetCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sreset_", u8CpuBaseName);
		ProcBegin();
		...
		fprintf(fp, "		mov	word [_z80ix], 0ffffh	; Default u8mz80Index register\n");
		fprintf(fp, "		mov	word [_z80iy], 0ffffh	; Default u8mz80Index register\n");
...
static void SetContextCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sSetContext_", u8CpuBaseName);
		ProcBegin();
...
static void GetContextCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sGetContext_", u8CpuBaseName);
		ProcBegin();
...
static void GetContextSizeCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sGetContextSize_", u8CpuBaseName);
		ProcBegin();
...
static void InitCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sinit_", u8CpuBaseName);
		ProcBegin();
	...
	if (MZ80_C == u8What)
	{
		...
		if (FALSE == u8StaticFlagTable)
		{
			...
			fprintf(fp, "\n");
			fprintf(fp, "		for (u32Loop = 0; u32Loop < (256*256*2); u32Loop++)\n");
			fprintf(fp, "		{\n");
			fprintf(fp, "			bLow = u32Loop & 0xff;\n");
			fprintf(fp, "			bHigh = (u32Loop >> 8) & 0xff;\n");
			fprintf(fp, "			bCarry = (u32Loop >> 16);\n");
			fprintf(fp, "\n");
			fprintf(fp, "			bFlag = 0;\n");
...
static void ShutdownCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sshutdown_", u8CpuBaseName);
		ProcBegin();
...
static void DebuggerCode(void)
{
	if (MZ80_ASSEMBLY_X86 == u8What)
	{
		...
		sprintf(u8ProcName, "%sSetRegisterValue_", u8CpuBaseName);
		ProcBegin();
		...
		sprintf(u8ProcName, "%sGetRegisterValue_", u8CpuBaseName);
		ProcBegin();
		...
		sprintf(u8ProcName, "%sGetRegisterName_", u8CpuBaseName);
		ProcBegin();
		...
		sprintf(u8ProcName, "%sGetRegisterTextValue_", u8CpuBaseName);
		ProcBegin();
		...
		sprintf(u8ProcName, "%sWriteValue_", u8CpuBaseName);
		ProcBegin();
		...
		sprintf(u8ProcName, "%sReadValue_", u8CpuBaseName);
		ProcBegin();
...
static void EmitCode(void)
{
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
		strcpy(u8mz80Index, "ix");
		
	else
	{
		strcpy(u8mz80Index, "cpu.z80IX");
		strcpy(u8mz80IndexHalfHigh, "cpu.z80XH");
		strcpy(u8mz80IndexHalfLow, "cpu.z80XL");
	}
	...
	if (MZ80_ASSEMBLY_X86 == u8What)
		strcpy(u8mz80Index, "iy");
	else
	{
		strcpy(u8mz80Index, "cpu.z80IY");
		strcpy(u8mz80IndexHalfHigh, "cpu.z80YH");
		strcpy(u8mz80IndexHalfLow, "cpu.z80YL");
	}
...
void main(int argc, char **argv)
{
	...
	if (argc < 2)
	{
		...
		printf("   -8085 - Emit only 8085 instructions\n");
		printf("   -sft  - Emit a static flag table (only for the C version)\n");
		exit(1);
	}
	...
	while (u32Loop < argc)
	{
		if (strcmp("-x86", argv[u32Loop]) == 0 || strcmp("-X86", argv[u32Loop]) == 0)
			u8What = MZ80_ASSEMBLY_X86;
		if (strcmp("-c", argv[u32Loop]) == 0 || strcmp("-C", argv[u32Loop]) == 0)
			u8What = MZ80_C;
		if (strcmp("-cs", argv[u32Loop]) == 0 || strcmp("-cs", argv[u32Loop]) == 0)
			u8ThroughCallHandler = TRUE;
		if (strcmp("-8085", argv[u32Loop]) == 0)
			u88085Mode = TRUE;
		if (strcmp("-s", argv[u32Loop]) == 0 || strcmp("-S", argv[u32Loop]) == 0)
			u8UseStack = 1;
		if (strcmp("-l", argv[u32Loop]) == 0 || strcmp("-L", argv[u32Loop]) == 0)
			u8Plain = TRUE;
		if (strcmp("-16", argv[u32Loop]) == 0)
			u816BitIO = TRUE;
		if (strcmp("-os2", argv[u32Loop]) == 0 || strcmp("-OS2", argv[u32Loop]) == 0)
			u8OS2 = TRUE;
		if (strcmp("-sft", argv[u32Loop]) == 0 || strcmp("-SFT", argv[u32Loop]) == 0)
			u8StaticFlagTable = TRUE;
		if (strcmp("-nt", argv[u32Loop]) == 0)
		{
			u8NoTiming = TRUE;
		}
	...
	if (u8What == MZ80_UNKNOWN)
	{
		fprintf(stderr, "Need emitted type qualifier\n");
		exit(1);
	}

	for (u32Loop = 1; u32Loop < argc; u32Loop++)
		if (argv[u32Loop][0] != '-')
		{
			fp = fopen(argv[u32Loop], "w");
			break;
		}
